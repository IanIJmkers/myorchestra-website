...
function DashboardCard({ title, items, more }) {
  return (
    <div className="bg-white rounded-xl border border-slate-200/60 overflow-hidden shadow-card hover:shadow-card-hover transition-shadow duration-300">
      ...
      <div className="p-4 min-h-[80px]">
        {items.map((item) => (
          <div key={item.text} className="flex items-start justify-between gap-3 mb-2 last:mb-0">
            <span className="text-xs text-slate-500 leading-snug">{item.text}</span>
            <span className="shrink-0 inline-flex items-center justify-center min-w-[22px] h-[22px] rounded-full bg-teal-500 text-white text-[10px] font-bold px-1.5">
              {item.count}
            </span>
          </div>
        ))}
        {more && (
          <p className="text-[11px] text-slate-400 mt-2">En {more} meer</p>
        )}
      </div>
    </div>
  );
...
function ModuleHeading({ icon, title }) {
  return (
    <h3 className="text-lg font-heading text-slate-900 mb-4 flex items-center gap-2">
      <SidebarIcon type={icon} className="w-5 h-5 text-slate-700" />
      {title}
    </h3>
  );
}
            ...
            <thead>
              <tr className="border-b border-slate-200">
                ...
                {balansData.years.map((y) => (
                  <th key={y} className="text-right px-4 py-2 text-slate-500 font-medium">{y}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              ...
              <tr className="border-t border-slate-200 font-semibold">
                ...
                {balansData.activaTotal.map((v, i) => (
                  <td key={i} className="px-4 py-2 text-right text-slate-900 tabular-nums">{v}</td>
                ))}
              </tr>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                <th className="text-left px-3 py-2 text-slate-500 font-medium">Grootboekrekening</th>
                <th className="text-right px-3 py-2 text-slate-500 font-medium">Bedrag (Euro)</th>
                <th className="text-left px-3 py-2 text-slate-500 font-medium">Kostenplaats</th>
              </tr>
            </thead>
            <tbody>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
        ...
        <div className="lg:col-span-2 bg-white rounded-xl border border-slate-200/60 shadow-card p-5">
          ...
          <div className="mt-4 h-2 bg-slate-100 rounded-full overflow-hidden">
            <div className="h-full bg-teal-500 rounded-full" style={{ width: "1.15%" }} />
          </div>
          ...
          <div className="border border-slate-200 rounded p-4 h-full bg-white">
            ...
            <div className="flex justify-between text-[9px] text-slate-500 mb-3">
              ...
              <div className="text-right space-y-0.5">
                ...
                <p><span className="text-slate-400">Factuurdatum</span> {invoicePreview.date}</p>
            ...
            <table className="w-full text-[9px] mb-3">
              <thead>
                <tr className="border-b border-slate-200">
                  <th className="text-left py-1 text-slate-500 font-medium">Beschrijving</th>
                  <th className="text-right py-1 text-slate-500 font-medium">Aantal</th>
                  <th className="text-right py-1 text-slate-500 font-medium">Eenheid</th>
                  <th className="text-right py-1 text-slate-500 font-medium">Tarief</th>
                  <th className="text-right py-1 text-slate-500 font-medium">Totaal</th>
                </tr>
              </thead>
              <tbody>
                {invoicePreview.lines.map((line) => (
                  <tr key={line.desc} className="border-b border-slate-50">
                    <td className="py-1 text-slate-600">{line.desc}</td>
                    <td className="py-1 text-right text-slate-600">{line.aantal}</td>
                    <td className="py-1 text-right text-slate-600">{line.eenheid}</td>
                    <td className="py-1 text-right text-slate-600">{line.tarief}</td>
                    <td className="py-1 text-right text-slate-600">{line.totaal}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <tbody>
              {relatiesData.map((row, i) => (
                <tr key={i} className="border-b border-slate-50 hover:bg-slate-50/50">
                  ...
                  <td className="px-3 py-2 text-slate-900 font-medium whitespace-nowrap">{row.naam}</td>
                  <td className="px-3 py-2 text-slate-500 whitespace-nowrap">{row.adres || "-"}</td>
                  <td className="px-3 py-2 text-slate-500">{row.postcode || "-"}</td>
          ...
          <table className="w-full text-xs min-w-162.5">
            ...
            <tbody>
              {table.map((row) => (
                <tr key={row.id} className="border-b border-slate-50 hover:bg-slate-50/50">
                  <td className="px-3 py-2 text-slate-400">{row.id}</td>
                  <td className="px-3 py-2 text-slate-500">{row.afkomst}</td>
                  <td className="px-3 py-2 text-slate-500">{row.donateur}</td>
                  <td className="px-3 py-2 text-slate-500 tabular-nums">€ {row.bedrag}</td>
                  <td className="px-3 py-2 text-slate-500">{row.datum}</td>
                  <td className="px-3 py-2 text-slate-500">{row.omschrijving}</td>
                  <td className="px-3 py-2 text-slate-500">{row.oormerk}</td>
...
function DocumentenContent() {
  ...
  return (
    <>
      ...
      <div className="bg-white rounded-xl border border-slate-200/60 shadow-card overflow-hidden">
        <div className="px-3 py-2 border-b border-slate-100 flex items-center justify-between">
          <span className="text-[10px] text-slate-400">1-{documentenData.length} van 134</span>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-xs min-w-162.5">
            <thead>
            ...
            <tbody>
              {documentenData.map((doc) => (
                <tr key={doc.id} className="border-b border-slate-50 hover:bg-slate-50/50">
                  ...
                  <td className="px-3 py-2 text-slate-900 font-medium whitespace-nowrap">{doc.naam}</td>
          ...
          <div className="p-4">
            ...
            {genodigden.map((g) => (
              <div key={g.naam} className="flex items-center text-xs gap-4 py-1.5">
                <span className="text-slate-900 font-medium grow">{g.naam}</span>
...
export default function MijnOrchestra() {
  const features = [
    { icon: featureIcons[0], title: "Overzichtelijk Dashboard", description: "Direct inzicht in alle belangrijke informatie van uw organisatie." },
    { icon: featureIcons[1], title: "Veilige Documentopslag", description: "Al uw documenten veilig bewaard en eenvoudig terug te vinden." },
    { icon: featureIcons[2], title: "Heldere Rapportages", description: "Financiële overzichten en rapportages in één oogopslag." },
    { icon: featureIcons[3], title: "Taakbeheer", description: "Houd uw taken bij en werk efficiënt samen met uw team." },
  ];
        ...
        <StaggerChildren className="mt-16 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {features.map((f) => (
            <motion.div key={f.title} variants={staggerItem}>
              <div className="bg-white border border-slate-200/60 rounded-2xl p-6 h-full shadow-card hover:shadow-card-hover transition-all duration-300 hover:-translate-y-1">
                <div className="text-teal-600 mb-4">{f.icon}</div>
                <h3 className="text-lg font-heading font-bold text-slate-900 mb-2">{f.title}</h3>
